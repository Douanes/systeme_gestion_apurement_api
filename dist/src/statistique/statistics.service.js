...
let StatisticsService = class StatisticsService {
    ...
    calculatePeriodDates(period) {
        ...
        const fin = new Date();
        switch (period) {
            case apurement_statistics_dto_1.PeriodFilter.TODAY:
                debut.setHours(0, 0, 0, 0);
                fin.setHours(23, 59, 59, 999);
                break;
            case apurement_statistics_dto_1.PeriodFilter.THIS_WEEK:
                ...
                fin.setHours(23, 59, 59, 999);
                break;
            case apurement_statistics_dto_1.PeriodFilter.THIS_MONTH:
                debut.setDate(1);
                debut.setHours(0, 0, 0, 0);
                fin.setMonth(now.getMonth() + 1, 0);
                fin.setHours(23, 59, 59, 999);
                break;
            case apurement_statistics_dto_1.PeriodFilter.THIS_YEAR:
                debut.setMonth(0, 1);
                debut.setHours(0, 0, 0, 0);
                fin.setMonth(11, 31);
                fin.setHours(23, 59, 59, 999);
                break;
            case apurement_statistics_dto_1.PeriodFilter.LAST_7_DAYS:
                debut.setDate(now.getDate() - 6);
                debut.setHours(0, 0, 0, 0);
                fin.setHours(23, 59, 59, 999);
                break;
            case apurement_statistics_dto_1.PeriodFilter.LAST_30_DAYS:
                debut.setDate(now.getDate() - 29);
                debut.setHours(0, 0, 0, 0);
                fin.setHours(23, 59, 59, 999);
                break;
            case apurement_statistics_dto_1.PeriodFilter.LAST_90_DAYS:
    ...
    async getApurementStatistics(query) {
        let dateDebut;
        let dateFin;
        if (query.period && query.period !== apurement_statistics_dto_1.PeriodFilter.CUSTOM) {
            ...
            dateDebut = query.dateDebut
                ...
                : this.calculatePeriodDates(apurement_statistics_dto_1.PeriodFilter.THIS_MONTH).debut;
            dateFin = query.dateFin
                ? new Date(query.dateFin)
                : this.calculatePeriodDates(apurement_statistics_dto_1.PeriodFilter.THIS_MONTH).fin;
        }
        const where = {
            deletedAt: null,
            dateOrdre: {
        ...
        const [totalOrdres, apures, nonApures, rejetes, bureauSortie] = await Promise.all([
    ...
    async getApurementByBureau(query) {
        ...
        const statistics = await Promise.all(bureaux.map(async (bureau) => {
            const where = {
                deletedAt: null,
    ...
    async getApurementTrend(query) {
        ...
        if (query.period && query.period !== apurement_statistics_dto_1.PeriodFilter.CUSTOM) {
            ...
            dateDebut = query.dateDebut
                ? new Date(query.dateDebut)
                : this.calculatePeriodDates(apurement_statistics_dto_1.PeriodFilter.THIS_YEAR).debut;
            dateFin = query.dateFin
                ? new Date(query.dateFin)
                : this.calculatePeriodDates(apurement_statistics_dto_1.PeriodFilter.THIS_YEAR).fin;
        }
        const where = {
            deletedAt: null,
            dateOrdre: {
        ...
        ordres.forEach((ordre) => {
            ...
            stats.totalOrdres++;
            if (ordre.statutApurement === 'APURE_SE')
                stats.apures++;
            else if (ordre.statutApurement === 'NON_APURE')
                stats.nonApures++;
            else if (ordre.statutApurement === 'REJET')
                stats.rejetes++;
        });
        return Array.from(grouped.values())
            .map((stat) => ({
            ...stat,
            tauxApurement: stat.totalOrdres > 0
    ...
    async getDashboardOverview(query) {
        ...
        return {
            ...
            comparison: {
                totalOrdresChange: this.calculatePercentageChange(previousPeriod.totalOrdres, currentPeriod.totalOrdres),
                tauxApurementChange: this.calculatePercentageChange(previousPeriod.tauxApurement, currentPeriod.tauxApurement),
            },
            topBureaux: byBureau
    ...
    async getPreviousPeriodStats(query) {
        let dateDebut;
        let dateFin;
        if (query.period && query.period !== apurement_statistics_dto_1.PeriodFilter.CUSTOM) {
            const currentDates = this.calculatePeriodDates(query.period);
            const duration = currentDates.fin.getTime() - currentDates.debut.getTime();
            ...
            return this.getApurementStatistics({
                period: apurement_statistics_dto_1.PeriodFilter.THIS_MONTH,
            });
        }
        return this.getApurementStatistics({
            ...query,
            ...
            period: apurement_statistics_dto_1.PeriodFilter.CUSTOM,
        });
    }
    ...
    async getMarchandiseByBureau(query) {
        ...
        const results = await Promise.all(bureaux.map(async (bureau) => {
    ...
    async getMarchandiseTrend(query) {
        ...
        const marchandiseCounts = new Map();
        colis.forEach((coli) => {
        ...
        colis
            ...
            .forEach((coli) => {
            if (!coli.ordreMission?.dateOrdre)
                return;
            let periodKey;
        ...
        return Array.from(grouped.values())
            ...
            poidsTotal: stat.poidsTotal > 0
                ? parseFloat(stat.poidsTotal.toFixed(2))
                : null,
            valeurTotale: stat.valeurTotale > 0
    ...
    async getDestinationStatistics(query) {
        ...
        const destinations = Array.from(grouped.values())
            ...
            .slice(0, limit);
        const totaux = Array.from(grouped.values()).reduce((acc, stat) => ({
    ...
    async getDestinationTrend(query) {
        ...
        ordres.forEach((ordre) => {
            const destination = ordre.destination || 'Non spécifiée';
            const totalTransports = ordre.conteneurs.length + ordre.camions.length + ordre.voitures.length;
        ...
        return Array.from(grouped.values())
            ...
            nombreTransports: stat.nombreConteneurs + stat.nombreCamions + stat.nombreVoitures,
        }))
            .sort((a, b) => {
            const periodCompare = a.period.localeCompare(b.period);
            if (periodCompare !== 0)
    ...
    async getRouteStatistics(query) {
        ...
        ordres.forEach((ordre) => {
            ...
            stats.ordreIds.add(ordre.id);
            const transports = ordre.conteneurs.length + ordre.camions.length + ordre.voitures.length;
